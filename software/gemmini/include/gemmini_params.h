...
/*
 * Local software header for GemminiFPConfigs.BF16Default8Config.
 *
 * The checked-in Gemmini software tree ships an int8 default header. This
 * benchmark needs the BF16 8x8 configuration so the generated Girdap BF16
 * tensors can be passed to Gemmini without quantizing them to int8.
 */
...
static const float elem_t_max = 3.38953139e38f;
static const float elem_t_min = -3.38953139e38f;
...
#define ACC_SCALE(x, scale) ((x) * (scale))
#define MVIN_SCALE(x, scale) ((x) * (scale))
#define MVIN_SCALE_ACC(x, scale) ((x) * (scale))

#define ACC_SCALE_T_IS_FLOAT
#define ACC_SCALE_EXP_BITS 8
#define ACC_SCALE_SIG_BITS 24
#define SCALE_T_IS_FLOAT
#define SCALE_EXP_BITS 8
#define SCALE_SIG_BITS 24
#define SCALE_ACC_T_IS_FLOAT
#define SCALE_ACC_EXP_BITS 8
#define SCALE_ACC_SIG_BITS 24
