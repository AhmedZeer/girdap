...
static void print_solution_sample_chunk(
  ...
  printf("\n");
  printf("SAMPLE_JSON {\"workload\":\"gpt2-prefill\",\"case\":%d,\"name\":\"%s\",\"tensor\":\"%s\",\"start\":%d,\"count\":%d,\"total\":%d,\"hw_rc\":%d,\"sw\":[",
...
int main(void) {
  ...
  printf("mode: %s  hardware: matmul=%d attention=%d softmax=%d\n",
         ...
         GIRDAP_HW_SOFTMAX);
  printf("opcodes: matmul=%d attention=%d softmax=%d  causal=row_prefix_calls\n",
         SA_MATMUL_OPCODE, SA_ATTN_OPCODE, SOFTMAX_OPCODE);
  printf("RUN_INFO_JSON {\"workload\":\"gpt2-prefill\",\"mode\":\"%s\",\"matmul_hw\":%d,\"attention_hw\":%d,\"softmax_hw\":%d,\"matmul_opcode\":%d,\"attention_opcode\":%d,\"softmax_opcode\":%d,\"causal\":\"row_prefix_calls\"}\n",
         GIRDAP_BENCHMARK_MODE, GIRDAP_HW_MATMUL, GIRDAP_HW_ATTENTION,
  ...
  for (int i = 0; i < gpt2_prefill_case_count; i++) {
    ...
    printf("\n[Case %d] %s\n", i, tc->name);
    printf("  shape : seq=%d d_model=%d heads=%d head_dim=%d hidden=%d layers=%d vocab=%d\n",
    ...
    printf("          embedding=%lu ln=%lu qkv=%lu attn=%lu attn_accel=%lu out=%lu fc1=%lu gelu=%lu fc2=%lu lm_head=%lu\n",
    ...
    printf("RESULT_JSON {\"workload\":\"gpt2-prefill\",\"case\":%d,\"name\":\"%s\",\"status\":\"%s\",\"hw_rc\":%d,\"raw_hw_rc\":%lu,\"mismatches\":%d,\"cpu_cycles\":%lu,\"hw_cycles\":%lu,\"speedup_x100\":%lu,\"hw_max_abs_diff_x100000\":%lu,\"cpu_ref_max_abs_diff_x100000\":%lu}\n",
           i, tc->name,
           (hw_rc == WS_GEMM_OK && mismatches == 0) ? "PASS" : "FAIL",
           hw_rc,
