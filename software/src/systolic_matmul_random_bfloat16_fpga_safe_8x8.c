...
int main(void) {
  ...
  static uint64_t a_tiles[WS_GEMM8_A_TILE_WORDS(MAX_M, MAX_K)] __attribute__((aligned(64)));
  static uint64_t b_tiles[WS_GEMM8_B_TILE_WORDS(MAX_N, MAX_K)] __attribute__((aligned(64)));
  static uint64_t c_words[WS_GEMM8_C_TILE_WORDS] __attribute__((aligned(64)));

  const ws_gemm_workspace_t workspace =
      ws_gemm8_make_workspace(a_tiles, b_tiles, c_words, MAX_M, MAX_N, MAX_K);
  ...
  srand(1);
  printf("=== Systolic GEMM Random Test (BF16 FPGA-Safe 8x8) ===\n");
  ...
  if (total_mismatches == 0) {
    printf("PASS: all randomized BF16 GEMM 8x8 cases matched the exact BF16 reference.\n");
  } else {
