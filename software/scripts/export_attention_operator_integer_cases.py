#!/usr/bin/env python3
"""Export integer-valued PyTorch attention cases as linkable C assets."""
...
DEFAULT_CASES = (
    AttentionCase("int_8x8x16x8", 8, 8, 16, 8, 301),
    AttentionCase("int_8x9x16x8", 8, 9, 16, 8, 302),
    AttentionCase("int_8x16x16x8", 8, 16, 16, 8, 303),
    AttentionCase("int_16x16x16x16", 16, 16, 16, 16, 304),
    AttentionCase("int_8x64x64x8", 8, 64, 64, 8, 305),
    AttentionCase("int_gpt2_decode_tile", 8, 256, 64, 64, 306),
    AttentionCase("int_gpt2_prefill_tile", 128, 128, 64, 64, 307),
    AttentionCase("int_vit_patch_attention", 197, 197, 64, 64, 308),
    AttentionCase("int_max_shape_stress", 256, 256, 256, 128, 309),
)
...
def make_case(case: AttentionCase, min_value: int, max_value: int):
    generator = torch.Generator(device="cpu")
    generator.manual_seed(case.seed)

    q = randint_tensor(generator, case.q_rows, case.d_k, min_value, max_value)
    k = randint_tensor(generator, case.kv_rows, case.d_k, min_value, max_value)
    v = randint_tensor(generator, case.kv_rows, case.value_cols, min_value, max_value)
...
def write_assets(
    ...
    generated = [(case, make_case(case, min_value, max_value)) for case in cases]

    print(f"INTEGER_RANGE,{min_value},{max_value}")
...
def main() -> None:
    ...
    parser.add_argument("--min-value", type=int, default=0, help="Minimum generated integer value.")
    parser.add_argument("--max-value", type=int, default=2, help="Maximum generated integer value.")
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    if args.min_value > args.max_value:
        raise SystemExit("--min-value must be <= --max-value")
    write_assets(DEFAULT_CASES, args.out_dir, args.min_value, args.max_value)
    print(
        f"wrote integer attention operator cases to {args.out_dir} "
        f"using range [{args.min_value}, {args.max_value}]"
    )
